...
/**
 * Tracks the eye positions and state over time, managing an underlying graphic which renders
 * eyes over the source video.<p>
 *
 */
class EyesTracker extends Tracker<Face> {
    private static final float EYE_CLOSED_THRESHOLD = 0.4f;

    private Overlay mOverlay;
    private EyesOverlayItem mEyesGraphic;
    ...
    EyesTracker(Overlay overlay, Context context) {
        mOverlay = overlay;
        mContext = context;
    }
    ...
    public void onNewItem(int id, Face face) {
        mEyesGraphic = new EyesOverlayItem(mOverlay, mContext);
    }
